#!/usr/bin/env python3
"""
flash_app.py  --  Streamlit GUI for the 4-way flash schedule checker.

Run locally:
    .env/bin/streamlit run flash_app.py

Run on a server so other computers on the network can reach it:
    .env/bin/streamlit run flash_app.py --server.address 0.0.0.0 --server.port 8501
    (a .streamlit/config.toml in this folder already sets these defaults)

Then open  http://<this-computer-ip>:8501  from any machine on the LAN.
"""
...
with tab_folder:
    ...
    if "folder_results" in st.session_state:
        render_results(st.session_state["folder_results"], pattern)
